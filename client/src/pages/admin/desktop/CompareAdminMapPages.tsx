...
import { adminComparePlaceList, compareList } from '@/utils/networks';
import {
  ComparePlaceProps,
...
const CompareAdminMapPages: React.FC = () => {
  ...
  const allPlaceToCompare = useQuery({
    queryKey: ['place-to-compare'],
    queryFn: async () => await adminComparePlaceList(),
  });
  ...
  return (
    <Flex align="end" gap="middle" vertical>
      <PlaceCompareList
        ...
        dataSource={allPlaceToCompare.data}
        columns={columnsUser}
      />
      <TransferList
        rowKey={({ placeId }) => placeId}
...
export default CompareAdminMapPages;
